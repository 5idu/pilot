...
func newStream(id string, buffSize int, replay, isAutoStream bool, eventlogLen map[string]int, onSubscribe, onUnsubscribe func(string, *Subscriber)) *Stream {
	return &Stream{
		ID:            id,
		AutoReplay:    replay,
	...
	go func(str *Stream) {
		for {
			...
			case event := <-str.event:
				if str.AutoReplay {
					length, ok := str.EventLogLen[str.ID]
					if !ok || length <= 0 {
						str.Eventlog.Add(event)
					} else {
						str.Eventlog.Cutting(event, length)
					}
				}
