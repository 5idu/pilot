...
type Server struct {
	...
	AutoReplay bool
	// Config event log length, default not limit
	EventLogLen     map[string]int
	eventLogLenLock sync.RWMutex
...
func New() *Server {
	return &Server{
		...
		AutoStream:  true,
		AutoReplay:  true,
		EventLogLen: make(map[string]int),
...
func (s *Server) CreateStream(id string) *Stream {
	...
	str := newStream(id, s.BufferSize, s.AutoReplay, s.AutoStream, s.EventLogLen, s.OnSubscribe, s.OnUnsubscribe)
	str.run()
...
func (s *Server) AddEventLogLen(id string, length int) {
	s.eventLogLenLock.Lock()
	defer s.eventLogLenLock.Unlock()
	s.EventLogLen[id] = length
}
